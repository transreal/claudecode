...
| | `ClaudeUpdateDocumentation` | 差分検出による自動更新・モード制御・Baseline 切替(直近更新／GitHub コミット版)・並列更新・サイクル再開(resumption)・失敗分類(システム的失敗/品質ゲート失敗)・docs/docs/ 重複検出 |
...
これらはいずれも内部的な信頼性強化であり、ユーザーが意識して呼び出す新しい公開 API はありません。通常の `ClaudeEval` / `ClaudeQuery` / `ClaudeUpdateDocumentation` の利用フローの中で自動的に適用されます。
