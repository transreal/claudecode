...
これは、API キーを設定していないメーター制 provider（`zai` を契約していない環境など）や、選んだ瞬間にローカル推論サーバへ接続にいく provider（`freetoken` / `llamacpp` など。常駐サーバを起こしたくない場合がある）を、環境ごとに候補から締め出すための仕組みです。
...
#### 20. ClaudeRuntime で「二重実行防止」により Input セルが自動評価されない

ClaudeRuntime を使ったコード実行で、LLM 応答からエージェントが再抽出したコードブロックが、runtime 側で既に実行済みの式と一致する（または実行済みコードの一部である）場合、Input セルへの書き込みは行われますが自動評価は抑制されます（二重実行防止ガード、2026-09-18 修正）。この場合、以下のようなメッセージが表示されます：

```
ℹ️ runtime が実行済みの式です。Input セルは書かれましたが自動評価はされません (二重実行防止)。必要なら手動で Shift+Enter してください。
```

このガードは、`GitHubCreateRepository` のような非冪等な操作を runtime が既に実行済みであるにもかかわらず、LLM 応答から再抽出したコードが誤って自動評価され、2 回目の実行が「422 name already exists」のようなエラーで失敗する事故を防ぐために追加されました。判定材料は「runtime state（`LastProposal["RawCode"]` や `LastExecutionResult` 付きの `ExecutionResult` メッセージ）から集めた実行済みコード集合」であり、誤判定は「自動評価しない」側に倒れる設計です（安全側）。
