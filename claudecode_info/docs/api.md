...
Options: `Fallback -> False`, `WebFetch -> False`, `WebSearch -> True`, `Model -> Automatic`, `PrivacySpec -> Automatic`, `AutoPrivate -> False`
...
Options: `AutoEvaluate -> True`, `StartTime -> Now`, `Fallback -> False`, `WebFetch -> Automatic`, `WebSearch -> True`, `RepeatInterval -> None`, `Model -> Automatic`, `PrivacySpec -> Automatic`, `AutoPrivate -> False`
...
ClaudeQuery/ClaudeEval のオプション。Claude Code CLI の組み込み Web 検索ツールを許可する。これは Claude Code 自体の Web 検索機能であり、API 経由の課金は発生しない。WebFetch（課金あり）とは異なる。
