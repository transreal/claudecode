...
| `$ClaudeWorkingDirectory` | String | `$HomeDirectory/Claude Working` | Claude Code の作業ディレクトリ |
...
| `$ClaudeFallbackModels` | List | `{{"anthropic","claude-opus-4-6"},{"openai","gpt-5"}}` | フォールバックモデル優先順位。各要素は `{"provider","model"}` |
...
| 関数 | 説明 |
|------|------|
| `ClaudeQuery[prompt]` | prompt を Claude Code に送信し応答文字列を返す（同期） |
...
セッションを継続する。引数なしは「エラーを修正してください」で継続。`StartTime` オプション対応。
