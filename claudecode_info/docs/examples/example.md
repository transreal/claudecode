...
### docs/docs/ ネスト重複ドキュメントの検出

過去のバグや手動操作により `docs/` フォルダの中にさらに `docs/` サブディレクトリが作られてしまうと、実質的に同じドキュメントが二重に存在する状態になります。ClaudeUpdateDocumentation は更新対象を確定する際にこの `docs/docs/` ネスト構造を検出し、該当ファイルを更新対象から自動的に除外します。

```mathematica
ClaudeUpdateDocumentation["myUtils"]
(* 検出時の出力例: *)
(* ⚠️ docs/docs/ にネストした重複ドキュメントを検出した (2 件)。更新対象から除外します。削除を推奨: docs/docs/setup.md, docs/docs/api.md *)
```

> 検出されたファイルは更新対象から自動的に除外されるだけで、削除は行われません。表示されたパスを確認し、不要であれば手動で削除することを推奨します。
...
更新チェーンにおける失敗は、**システム的失敗**と**品質失敗**の 2 種類に分類され（2026-07-09 改訂）、それぞれ扱いが異なります。

- **システム的失敗**（API エラー／利用制限／プロバイダ内部エラー／空応答）を検出すると、従来どおりその時点でチェーン全体を即座に中断します（fail-fast）。原因を解消してから再実行すると、記録済みの進捗をもとに中断したドキュメントから再開されます。
- **品質失敗**（応答の切り詰め＝truncation／サイズ退行／タイトル不整合）は、検出しても直ちにチェーン全体を中断せず、まず同一ファイルの再生成をその場で試みます（最大1回）。再生成時には「単一応答で出力しツールは使用しないこと、コードフェンスの対応を崩さないこと」を指示する RETRY NOTICE がプロンプトに追加注入されます。

```mathematica
ClaudeUpdateDocumentation["myUtils"]
(* 品質失敗を検出し再生成を試みる場合の出力例: *)
(* ⚠ [3/7] api.md の生成が未完了 [切り詰め]。再生成します (1/1) *)
(* 再生成後も品質失敗が続いた場合の出力例: *)
(* ⛔ [3/7] api.md をスキップ (無効/切り詰め/サイズ退行/タイトル不整合)。ファイルは変更せず次のファイルへ進みます *)
```

> 再生成を試みても品質失敗が解消しない場合、そのファイルは**更新せずスキップ**され、残りのドキュメントの更新はそのまま継続されます。以前は品質失敗もチェーンを即中断していたため、持続的に失敗する 1 ファイル（例: 巨大化した setup.md）が残り全部の更新を永久にブロックしてしまう事故が起きていました。この分類変更により、その事故を避けられます。スキップされたファイルは進捗としては未成功のまま残るため、次回実行時に改めて更新対象となります。不完全なドキュメントで既存ファイルを上書きすることはありません。

なお、サイズ退行は書き込み直前の安全弁でも検出されます。既存ファイルが存在する場合、新しい内容の文字数が既存の **40% 未満**なら「サイズ退行」として書き込みを拒否し（品質失敗として扱う）、既存ファイルを保護します。

```mathematica
(* サイズ退行を検出した場合の出力例: *)
(* ⚠ iSafeWriteDoc: サイズ退行を検出 (api.md): 12000→3000文字(25%, 閾値 40%) *)
```
...
- 反映した変更（2026-07-07〜07-09）: セクション 22「品質ガードと切り詰め検出」を **失敗分類（システム的失敗＝fail-fast／品質失敗＝再生成→スキップ）** の 2 分類として整理し、2026-07-09 の改訂に合わせて出力例（`⚠ … の生成が未完了 [切り詰め]。再生成します`／`⛔ … をスキップ …`）を実際のメッセージ文言に寄せ、**サイズ退行の 40% 閾値**（`iSafeWriteDoc` の書き込み拒否）を追記しました。
- 追加/削除した公開関数・オプション: 差分はフリーズ対策・バックアップ差分比較（同名除外）・FE 応答性 probe・ログローテート・aux ハッシュ／並列カーネル周りの**内部リファクタリング**が中心で、公開関数・公開オプションの新規追加や削除は含まれていなかったため、関数説明の追加・削除は行っていません（`Baseline`、`$ClaudePackageAuxKeywordMap`、`$ClaudeCLIMCPServers`、Spec 系関数などは既に記載済み）。