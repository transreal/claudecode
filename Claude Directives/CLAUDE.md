...
- **生成済み成果物の照会は SourceVault ファースト** (`rules/22-artifact-retrieval-vault-first.md`)。採点・質問・要約など生成済み成果物の照会は、まず生成元パッケージの照会 API (`CerezoGradeReport[url]` / `CerezoQuestionReport[url]` / `sourcevault_search` 等) で SourceVault の正本から引く。xlsx 等のエクスポートは派生物。**`$ClaudeAccessibleDirs` の変更・ファイル直読みは経路を試し尽くした後の最後の手段**で、理由を明記する。
- **モデル選択の既定と課金 API の鉄則** (`rules/21-model-selection-and-paid-api.md`)。何の指示もない場合、モデルは privacy level に従って `$ClaudeModel` (PL<0.5) / `$ClaudePrivateModel` (PL>=0.5) を使う（`ClaudeQuerySync` 等の privacy 対応ルータに委ねる）。既定以外のモデルや別チャネル (`LLMSynthesize` = Wolfram AI Access 等) を使うときは明記して切り替える。**課金 API 許可フラグ (`NBAccess`NBGetNotebookPaidAPIAllowed`、既定 False=禁止) は privacy level より上位で、あらゆるモデル選択の決定を支配する**。禁止なら PrivacyLevel に関係なく課金 API へは送らない。
...
- `dynamic-panel-render-cost` — DynamicModule/Dynamic ベースの一覧パネル (SourceVault ワークフロー一覧・保存プロンプト一覧) が「一瞬フリーズ→$Aborted」で中身が出ない事故の診断と修正。DynamicModule body / Dynamic[...] 描画の評価は FE 予算で外部 Abort される (通常評価/Queued ボタンは abort されない)。鉄則: (1) 重いデータは公開関数の通常評価で先算出し焼き込み (ExpressionCell は構築時に評価)・再取得は SessionSubmit、(2) **Grid 描画 Dynamic の中で行ごとに全件走査関数を呼ばない** (実際の真因: per-row `SourceVaultAutoTriggerStatusCell` が重い `ListData`+`NextFire` 地平線スキャンで 18 秒)、不要フィールド計算を外す/registry を短 TTL キャッシュ、(3) **headless で再現しないなら FE だけがロードする副パッケージ (autotrigger 等) の差分を疑う**、(4) **常時表示パレットの Dynamic に UpdateInterval/RefreshRate の常駐タイマを置かない** (別ノートブックの長時間 ClaudeEval 等でカーネル占有中に preemptive 評価が滞留→パレット操作で「動的評価の放棄」ダイアログ+FE 全体フリーズ; in-cell $Aborted と別症状)。常時パレットは SynchronousUpdating->False+明示 TrackedSymbols にし、更新はクリック時の自前ローカル変数/tick (ラベルは自前変数化=SlideWorkflow/documentation 方式、一覧/状態は DynamicModule ローカル tick=ClaudeProcessList 方式)。診断レシピ (RowsResult/PanelHead/NumTasks) 付き
...
- `sourcevault-source-summaries` — SourceVault.wl の ingest 済みソース行 (SourceVaultSources/SourceVaultArXiv) の Summary 列を埋める設計 (2026-06-24)。arXiv は ingest 時にアブストラクトを arXiv API から取得し `$Language` へ翻訳して meta["Summary"] 格納 (iIngestURL フック・同期/非同期両カバー・TimeConstrained best-effort)。既存は SourceVaultBackfillArXivSummaries[] で backfill。翻訳は cloud LLM (arXiv 公開→PL 0.0)、iSVLooksLikeLLMError ゲートで利用制限/エラー本文の保存を防ぐ、失敗時は原文 fallback。表示パス (iSVSourcesEnrich) は LLM-free に保つ原則と、**headless $Language=English 罠** (backfill は FE セッションで実行) を含む
...
- `sourcevault-packageapi-narrowing` — このシステム自身のパッケージ (SourceVault/ClaudeOrchestrator/ClaudeRuntime/NBAccess/claudecode/github) で関数・実装箇所を探す初動で、`$packageDirectory` 全体の広い `rg` でなく **SourceVault MCP packageapi マイニング** (`sourcevault_search kinds=["packageapi"]`) を先に使う手順。1 呼び出し 1 概念トークンで反復 (自然文/交替束を詰めない)、`sourcevault_get view=body` (grant 不要) で確定、`StaleDocs` は実装確認、`filters.packages` で package scope、条件付き (0 件/StaleDocs 上位/timeout) で glob 境界付き `rg` fallback。根拠は `ドキュメント/sourcevault_mining_narrowing_vs_fulltext_proposal_v0_1.md`
- `sourcevault-service-mcp-lifecycle` — SourceVault の detached service (WL カーネル) + HTTP/MCP proxy (Python) の起動・停止・復帰診断。**「MCP 停止中のままクリックしても戻らない」最頻出事故の真因**: カーネル crash 後も `status.json` が `State:"Running"` のまま残り、`SourceVaultStartMCP` が State 文字列だけで AlreadyRunning と誤認して再起動をスキップ → proxy だけ上がり `/health` が Stale → `SourceVaultMCPRunningQ` False → 復帰不能。対策は `svcRunning` に `PidAlive` 条件を AND (status.json の State 単独を信用しない)。runtime はマシン別 (`runtime/<MachineTag>/services|proxies`) で症状は各 PC 独立に再発。即時復帰 `SourceVaultStartMCP["RestartService"->True]`、恒久対策 `SourceVaultInstallWatchdog`。診断は status.json の State と heartbeat.json の鮮度・pid 生存を必ず突き合わせる (2026-06-24)
