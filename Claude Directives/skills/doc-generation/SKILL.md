...
### README.md の境界ルール（必須）
- README.md は「免責事項」→「ライセンス」で終了する。それ以降にコンテンツを追加してはならない。
- `ClaudeUpdateDocumentation` の第二引数（指示文）やノートブックコンテキストに、他ドキュメント向けの更新内容（例: user_manual.md に追記すべき項目リスト）が含まれている場合、**それらの詳細を README.md にコピーしてはならない**。
- README.md に反映してよいのは、パッケージの概要・機能一覧・構造に影響する変更のみ。指示文中のプロンプト断片や他ドキュメント向けの具体的な記述をそのまま README に転記するのは誤りである。
