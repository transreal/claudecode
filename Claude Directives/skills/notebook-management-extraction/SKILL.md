...
notebook source の鮮度管理 (SourceVaultSync)、ファイル移動追跡 (SourceVaultRelinkSources)、移動に強い同定キーの埋め込み (Notebook UUID) は、独立した skill **`skills/sourcevault-sync-relink-uuid`** に分離した。notebook crawling / staleness / relink / UUID を扱うときはそちらを参照。
