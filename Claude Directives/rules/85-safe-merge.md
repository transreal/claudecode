...
- 4段階フォールバックで挿入位置を決定する:
  1. `End[]` + `EndPackage[]` が隣接する標準構造の直前
  2. 最後の `End[]` の直前（`EndPackage[]` が先頭付近にある非標準構造に対応）
  3. `StringPosition` で最後の `End[]` を探して直前に挿入
  4. 上記すべて不成功なら末尾に追加
- **挿入の成否を必ず検証する**（`code =!= codeBefore`）。`StringReplace` がマッチしなかった場合はサイレントに失敗するため、検証なしでは挿入失敗が隠蔽される
- 挿入した関数名をユーザーに通知する。挿入失敗時は警告を表示する
- 詳細な実装パターンは `skills/package-merge-pattern` を参照
